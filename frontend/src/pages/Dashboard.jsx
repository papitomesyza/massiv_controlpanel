...
function ChartTooltip({ active, payload, label, labelFmt }) {
  ...
  return (
    <div style={{
      ...
      {payload.map(p => (
        <div key={p.dataKey} style={{ color: '#fff', fontWeight: 600 }}>
    ...
    <div className="dash-leads-band">
      ...
      {leads.length === 0 ? (
        <div className="dash-leads-empty">
          <Lightbulb size={22} style={{ color: '#333' }} />
    ...
    <Link to={`/projects/${p.id}`} style={{ textDecoration: 'none', display: 'block' }}>
      <div className="card project-dash-card">
        ...
        <div style={{ fontSize: '12px', color: '#888', marginBottom: '10px' }}>
          ...
          {p.category_name && <span style={{ color: '#555', margin: '0 5px' }}>·</span>}
          {p.category_name && <span>{p.category_name}</span>}
        </div>

        {/* Phase progress */}
        <div className="flex-between" style={{ fontSize: '11px', color: '#666', marginBottom: '4px' }}>
        ...
        {p.agreed_budget > 0 && (
          <>
            <div className="flex-between" style={{ fontSize: '11px', color: '#666', marginBottom: '4px' }}>
              <span>Received</span>
              <span>{fmt(p.total_received)} <span style={{ color: '#444' }}>/ {fmt(p.agreed_budget)}</span></span>
            </div>
