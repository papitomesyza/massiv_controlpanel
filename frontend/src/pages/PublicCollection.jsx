...
function FaviconPlaceholder({ url }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: '12px', color: '#737373', fontWeight: 500, maxWidth: '90%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textAlign: 'center' }}>
        {domain}
      </span>
    </div>
  );
...
function PublicLinkCard({ card }) {
  ...
  return (
    <div
      style={{
        background: '#ffffff', border: '1px solid #e5e5e5', borderRadius: 24,
    ...
    >
      ...
      <div style={{ padding: '11px 14px 13px', display: 'flex', flexDirection: 'column', gap: '5px', flex: 1 }}>
        {card.title ? (
          <p style={{ fontSize: '13px', fontWeight: 600, lineHeight: 1.4, margin: 0, color: '#0a0a0a', wordBreak: 'break-word' }}>
            {card.title}
          </p>
        ) : (
        ...
        {tags.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '3px' }}>
            {tags.map(tag => (
              <span key={tag} style={{ padding: '2px 9px', borderRadius: 18, fontSize: '11px', fontWeight: 600, background: 'rgba(10,10,10,0.05)', color: '#0a0a0a' }}>
                {tag}
              </span>
            ))}
          </div>
...
function PublicNoteCard({ card }) {
  ...
  return (
    <div style={{
      background: 'rgba(10,10,10,0.03)', border: '1px solid #e5e5e5', borderRadius: 24,
      ...
      {card.title && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
          ...
          <span style={{ fontSize: '13px', fontWeight: 700, color: '#0a0a0a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {card.title}
          </span>
        </div>
      )}
      <p style={{
        fontSize: '13px', lineHeight: 1.65, color: '#737373', margin: 0,
      ...
      {isLong && (
        <button
          ...
          style={{ alignSelf: 'flex-start', fontSize: '11px', fontWeight: 700, color: '#0a0a0a', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
        >
    ...
    return (
      <div style={{ ...page, alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <Lock size={40} color="#a3a3a3" style={{ marginBottom: 16 }} />
          <p style={{ fontSize: '18px', fontWeight: 700, marginBottom: 8 }}>This link has been revoked.</p>
          <p style={{ color: '#737373', fontSize: '14px' }}>The owner has disabled access to this shared collection.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ ...page, alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#737373', fontSize: '14px' }}>This shared collection link is invalid or no longer exists.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={page}>
      ...
      <div style={container}>
        ...
        {/* Card grid */}
        {cards.length === 0 ? (
          <div style={{ padding: '48px 20px', textAlign: 'center', color: '#737373', fontSize: '14px' }}>
