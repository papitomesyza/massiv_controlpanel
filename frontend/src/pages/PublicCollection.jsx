...
function FaviconPlaceholder({ url }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: '12px', color: 'var(--color-mid-gray)', fontWeight: 500, maxWidth: '90%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textAlign: 'center' }}>
        {domain}
      </span>
    </div>
  );
...
function PublicLinkCard({ card }) {
  ...
  return (
    <div
      style={{
        background: 'var(--surface-card)', border: '1px solid var(--color-hairline)', borderRadius: 24,
    ...
    >
      ...
      <div style={{ padding: '11px 14px 13px', display: 'flex', flexDirection: 'column', gap: '5px', flex: 1 }}>
        {card.title ? (
          <p style={{ fontSize: '13px', fontWeight: 600, lineHeight: 1.4, margin: 0, color: 'var(--color-ink)', wordBreak: 'break-word' }}>
            {card.title}
          </p>
        ) : (
        ...
        {tags.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '3px' }}>
            {tags.map(tag => (
              <span key={tag} style={{ padding: '2px 9px', borderRadius: 18, fontSize: '11px', fontWeight: 600, background: 'var(--overlay-04)', color: 'var(--color-ink)' }}>
                {tag}
              </span>
            ))}
          </div>
...
function PublicNoteCard({ card }) {
  ...
  return (
    <div style={{
      background: 'var(--overlay-02)', border: '1px solid var(--color-hairline)', borderRadius: 24,
      ...
      {card.title && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
          ...
          <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--color-ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {card.title}
          </span>
        </div>
      )}
      <p style={{
        fontSize: '13px', lineHeight: 1.65, color: 'var(--color-mid-gray)', margin: 0,
      ...
      {isLong && (
        <button
          ...
          style={{ alignSelf: 'flex-start', fontSize: '11px', fontWeight: 700, color: 'var(--color-ink)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
        >
    ...
    return (
      <div style={{ ...page, alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <Lock size={40} color="var(--color-faint)" style={{ marginBottom: 16 }} />
          <p style={{ fontSize: '18px', fontWeight: 700, marginBottom: 8 }}>This link has been revoked.</p>
          <p style={{ color: 'var(--color-mid-gray)', fontSize: '14px' }}>The owner has disabled access to this shared collection.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ ...page, alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--color-mid-gray)', fontSize: '14px' }}>This shared collection link is invalid or no longer exists.</p>
        </div>
      </div>
    );
  }
      ...
      <div style={container}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          {collection.description && (
            <p style={{ fontSize: '14px', color: 'var(--color-mid-gray)', margin: 0, lineHeight: 1.6 }}>
              {collection.description}
            </p>
          )}
        ...
        {/* Card grid */}
        {cards.length === 0 ? (
          <div style={{ padding: '48px 20px', textAlign: 'center', color: 'var(--color-mid-gray)', fontSize: '14px' }}>
