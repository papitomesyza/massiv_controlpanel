    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
        ...
        <div style={{ width: '38%', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {providers.length === 0 && archivedProviders.length === 0 && (
            <div className="card" style={{ padding: '48px 24px', textAlign: 'center' }}>
              ...
              <div style={{ color: '#fff', fontWeight: 600, marginBottom: '8px' }}>No providers yet</div>
              <div style={{ color: '#555', fontSize: '13px', marginBottom: '20px' }}>Add your first asset provider to get started.</div>
            ...
            <div style={{ marginTop: '4px' }}>
              <button
                ...
                style={{
                  ...
                  color: '#555', fontSize: '12px', padding: '6px 2px', width: '100%',
                }}
              >
        ...
        <div className="card" style={{ flex: 1, minHeight: '420px', display: 'flex', flexDirection: 'column' }}>
          {!selectedProvider ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, padding: '64px 24px', color: '#555' }}>
              <Package size={40} color="#2a2a2a" style={{ marginBottom: '12px' }} />
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--border)', gap: '12px', flexWrap: 'wrap' }}>
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: '#666', marginTop: '2px' }}>{selectedProvider.type}</div>
                </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {filteredItems.length === 0 ? (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr>
                        <th style={{ textAlign: 'left', padding: '10px 20px', fontSize: '11px', color: '#555', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--border)' }}>Item Name</th>
                        <th style={{ textAlign: 'left', padding: '10px 12px', fontSize: '11px', color: '#555', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--border)' }}>Category</th>
                        <th style={{ textAlign: 'right', padding: '10px 12px', fontSize: '11px', color: '#555', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--border)' }}>Daily Rate €</th>
                        <th style={{ textAlign: 'left', padding: '10px 12px', fontSize: '11px', color: '#555', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--border)', width: '160px' }}>Notes</th>
                          ...
                          <td style={{ padding: '10px 12px', textAlign: 'right' }}>
                            {editingRate?.id === item.id ? (
                              ...
                              <span
                                ...
                                style={{ cursor: 'pointer', fontWeight: 600, fontSize: '13px', padding: '3px 10px', borderRadius: '6px', background: 'rgba(255,255,255,0.04)', display: 'inline-block' }}
                              >
                                €{Number(item.daily_rate || 0).toFixed(0)}
                              </span>
                            )}
                          </td>
                          <td style={{ padding: '10px 12px', fontSize: '12px', color: '#555', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {item.notes || <span style={{ color: '#333' }}>—</span>}
                          </td>
                          <td style={{ padding: '10px 12px' }}>
                            <button
                              className="btn-icon"
        ...
        <div style={{ marginTop: '8px', display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
          ...
          {waLink && (
            <a
              ...
              style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#25D366', textDecoration: 'none' }}
          ...
          <div className="form-row" style={{ position: 'relative' }}>
            ...
            {dropdownOpen && (filtered.length > 0 || showCreateOption) && (
              <div style={{
                ...
                background: '#1e1e1e', border: '1px solid var(--border)', borderRadius: '8px',
                boxShadow: '0 8px 24px rgba(0,0,0,0.6)', marginTop: '4px', overflow: 'hidden',
              }}>
                {filtered.map(item => (
                  <div
                    key={item.id}
                    onMouseDown={() => pick(item)}
                    style={{ padding: '9px 14px', cursor: 'pointer', fontSize: '13px', borderBottom: '1px solid rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', gap: '8px' }}
                  >
                    <span style={{ flex: 1 }}>{item.name}</span>
                    {item.category && <span style={{ color: '#555', fontSize: '11px' }}>{item.category}</span>}
                  </div>
                ))}
                {showCreateOption && (
                  <div
