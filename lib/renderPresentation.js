...
// Pitch copy never carries a long dash: an em dash or horizontal bar becomes a
// comma, an en dash between two numbers becomes " to ", and any other en dash
// a comma. The AI polish endpoint runs its output through this too, and every
// rendered string passes through it on the way to esc(), so a pitch written
// before the rule still reads clean.
...
function esc(v) {
  ...
  return (/[\u2013\u2014\u2015]/.test(raw) ? scrubDashes(raw) : raw)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
...
/* 15vw so a phone title fills its frame instead of floating; the 7rem cap
   means desktop sizing is unchanged. */
.hero-title{font-size:clamp(2.9rem,15vw,7rem);font-weight:700;letter-spacing:-0.022em;line-height:1.0;color:#f5f5f7}
...
@media (max-width:640px){
  ...
  /* Vertical rhythm is tuned for a wide canvas; on a phone the same clamp
     leaves ~254px of dead space between sections. Roughly halve it. */
  .sec{padding:clamp(46px,7vh,66px) 22px}
  .sec-bleed{padding:clamp(24px,4vh,40px) 0}
  /* Let the next section peek so the hero reads as an opener, not a dead column */
  .hero{min-height:92vh;min-height:92dvh}
  .hero-inner{padding:24px 22px}
  .hero-eyebrow{margin-bottom:18px}
  .hero-sub{margin-top:16px}
  .h2{margin-bottom:18px}
  .mood-note{margin-top:20px}
  .cta{padding-bottom:0}
  .cta-brand{margin-top:40px}
}
/* Short viewports (landscape phones): never force a full-height hero */
...
<!-- Confidential client pitches: never index them, on either domain. Link
     preview crawlers (WhatsApp, iMessage, Slack) ignore this and still render
     the og: tags below, which is exactly what we want. -->
...
module.exports = { renderPresentation, SECTION_TYPES, scrubDashes };
