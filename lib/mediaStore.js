...
// original is never written to disk), images only, 25MB ceiling, one file.
...
const UPLOAD_ERRORS = {
  LIMIT_FILE_SIZE: [413, 'Image too large (max 25MB)'],
  UNSUPPORTED_TYPE: [415, 'Only jpeg, png, webp or gif images'],
  LIMIT_FILE_COUNT: [400, 'One image at a time'],
  LIMIT_UNEXPECTED_FILE: [400, 'One image at a time'],
};

// The single image upload every endpoint uses. multer errors never reach the
// app's missing error handler: a file over the size cap, a wrong type or an
// extra file each answer with a clear status and message, and a request with
// no file answers 400.
function singleImageUpload(field = 'image') {
  const handler = require('multer')(imageUploadOptions()).single(field);
  return (req, res, next) => {
    handler(req, res, err => {
      if (err) {
        const [status, message] = UPLOAD_ERRORS[err.code] || [400, 'Upload failed'];
        return res.status(status).json({ error: message });
      }
      if (!req.file) return res.status(400).json({ error: 'Image file required (jpeg, png, webp or gif, max 25MB)' });
      return next();
    });
  };
}
