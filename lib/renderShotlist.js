...
// except the shot list's own media under /shotlist-media. Every user string goes
// through esc().
...
// break out of /shotlist-media/, same guard the serving route applies.
function mediaSrc(filename) {
  if (!filename || typeof filename !== 'string') return null;
  if (/[/\\]/.test(filename) || filename.includes('..')) return null;
  return `/shotlist-media/${encodeURIComponent(filename)}`;
}
...
// What the recce found at this location. It sits on the scene because it
// describes the place rather than a framing, the angles are on the shots.
function scoutStrip(photos) {
  const list = (photos || []).map(m => {
  ...
  return `<div class="scout">
    <div class="scout-k">${icon('location')}Scout photos</div>
    <div class="media scout-shots">${list}</div>
  </div>`;
}
...
/* When the scene is actually on set. Outlined so it reads as the schedule
   rather than as one more property of the scene. */
.tag.time{background:var(--bg);color:var(--ink);box-shadow:inset 0 0 0 2px var(--ink)}
...
/* scout, the location as the recce found it */
.scout{padding:10px 14px 4px;border-top:1px solid var(--line-2)}
.scout-k{display:flex;align-items:center;gap:6px;font-size:12px;font-weight:700;
  letter-spacing:0.08em;text-transform:uppercase;color:var(--ink-3);margin-bottom:6px}
.scout-shots{grid-template-columns:repeat(auto-fill,minmax(132px,1fr));margin-top:0}
.scout-shots figcaption{text-transform:none;letter-spacing:0;font-weight:600;font-size:12px}
...
/* set design, edited from the page by unlocked crew */
.set-text:empty::before{content:'Nothing set yet';color:var(--ink-3)}
/* Its own line under the value, never trailing the last word of it. */
.set-edit{margin-top:8px;min-height:40px;padding:0 12px;border-radius:10px;border:1px solid var(--line);
  background:var(--chip);color:var(--ink);font-size:14px;font-weight:700;
  display:flex;width:fit-content;align-items:center;gap:6px}
.set-input{width:100%;border-radius:12px;border:1px solid var(--line);background:var(--bg);color:var(--ink);
  padding:10px 12px;font-size:16px;font-family:inherit;line-height:1.45;resize:vertical}
.set-actions{display:flex;gap:8px;margin-top:8px}
.set-actions .btn{flex:1;min-height:44px}
.set-err{color:#b3261e;font-size:14px;font-weight:650;margin-top:6px}
html[data-theme="dark"] .set-err{color:#ff8a80}
.set-saving{opacity:.55}
...
/* passcode sheet */
.notice{position:fixed;left:50%;bottom:calc(18px + env(safe-area-inset-bottom));transform:translateX(-50%);z-index:80;
  max-width:calc(100% - 32px);padding:11px 16px;border-radius:12px;background:var(--ink);color:var(--bg);
  font-size:14px;font-weight:600;box-shadow:0 6px 24px rgba(0,0,0,0.25)}
...
// Crew behaviour, ticking shots off and writing set design, is emitted ONLY
// when the shot list has a passcode. With no passcode there are no crew
// controls at all and no code to drive them.
function crewJs() {
  return `
  // What to do once the passcode has been accepted: the tap that opened the
  // sheet is replayed rather than lost.
  ...
  // Everything an unlocked crew member gets that a plain reader does not: the
  // Edit buttons, and the set design rows that are empty and therefore hidden
  // from a reader who could do nothing with them.
  function enableCrewMode(){
  ...
  // A failed tick is said in a short notice at the bottom of the screen, never
  // in a browser dialog.
  ...
  function toggleShot(el){
    var saved = crew();
    if (!saved || !saved.token) { pendingAction = function(){ toggleShot(el); }; openSheet(); return; }
    ...
    post('/api/public/shotlist/' + encodeURIComponent(SLUG) + '/shots/' + encodeURIComponent(shotId) + '/complete',
      ...
      .then(function(r){
        ...
        if (r.status === 401) { clearCrew(); pendingAction = function(){ toggleShot(el); }; openSheet(); return; }
        showNotice((r.data && r.data.error) || 'Could not update this shot.');
      })
      .catch(function(){ showNotice('Network problem. Try again.'); })
  ...
  // ── Set design, written on the day ──
  // The scene owns the dressing; a shot may carry a deviation from it. Both are
  // the same field on two tables, so both are edited the same way.

  // Show a saved value everywhere it appears. The scene's set design is
  // repeated inside each of its shots, so a save has to reach those copies or
  // the page contradicts itself until a reload.
  function applySetValue(row, value){
    var textEl = row.querySelector('.set-text');
    if (textEl) textEl.textContent = value;
    row.classList.remove('hidden');
    var rows = row.closest('.rows');
    if (rows) rows.classList.remove('hidden');
    if (row.getAttribute('data-set-target') !== 'scene') return;
    var id = row.getAttribute('data-set-id');
    document.querySelectorAll('[data-set-echo="' + id + '"]').forEach(function(echo){
      var t = echo.querySelector('.set-text');
      if (t) t.textContent = value;
      echo.classList.toggle('hidden', !value);
    });
  }

  function saveSet(row, ta, err, saveBtn, endEdit){
    var saved = crew();
    if (!saved || !saved.token) {
      pendingAction = function(){ saveSet(row, ta, err, saveBtn, endEdit); };
      openSheet();
      return;
    }
    err.textContent = '';
    saveBtn.disabled = true;
    ta.classList.add('set-saving');
    post('/api/public/shotlist/' + encodeURIComponent(SLUG) + '/set-design', {
      token: saved.token,
      target: row.getAttribute('data-set-target'),
      id: Number(row.getAttribute('data-set-id')),
      value: ta.value
    })
      .then(function(r){
        if (r.ok) { applySetValue(row, (r.data && r.data.value) || ''); endEdit(); return; }
        // A dead token must not cost the crew what they typed: keep the box
        // open, unlock again, then save exactly the same text.
        if (r.status === 401) {
          clearCrew();
          pendingAction = function(){ saveSet(row, ta, err, saveBtn, endEdit); };
          openSheet();
        ...
        err.textContent = r.status === 429
          ? 'Too many edits in a minute. Wait a moment and save again.'
          : 'Could not save this. Try again.';
      })
      .catch(function(){ err.textContent = 'Network problem. Try again.'; })
      .finally(function(){ saveBtn.disabled = false; ta.classList.remove('set-saving'); });
  }

  function beginEdit(row){
    if (row.querySelector('.set-input')) return;
    var box = row.querySelector('.row-v');
    var textEl = row.querySelector('.set-text');
    var editBtn = row.querySelector('.set-edit');
    if (!box) return;

    var ta = document.createElement('textarea');
    ta.className = 'set-input';
    ta.rows = 3;
    ta.maxLength = Number(row.getAttribute('data-set-max')) || 2000;
    ta.value = textEl ? textEl.textContent : '';
    ...
    save.type = 'button'; save.className = 'btn btn-solid'; save.textContent = 'Save';
    actions.appendChild(cancel); actions.appendChild(save);

    if (textEl) textEl.classList.add('hidden');
    if (editBtn) editBtn.classList.add('hidden');
    box.appendChild(ta); box.appendChild(err); box.appendChild(actions);
    ta.focus();

    function endEdit(){
      ta.remove(); err.remove(); actions.remove();
      if (textEl) textEl.classList.remove('hidden');
      if (editBtn) editBtn.classList.remove('hidden');
    }
    cancel.addEventListener('click', endEdit);
    save.addEventListener('click', function(){ saveSet(row, ta, err, save, endEdit); });
  }

  document.querySelectorAll('.set-edit').forEach(function(btn){
    btn.addEventListener('click', function(){
      var row = btn.closest('.row-set');
      if (!row) return;
      var saved = crew();
      if (!saved || !saved.token) { pendingAction = function(){ beginEdit(row); }; openSheet(); return; }
      beginEdit(row);
    });
  });
  ...
  // Unlocked once on this device, still unlocked. A token the server has since
  // stopped accepting, the passcode was changed, only shows up on the first
  // save, which reopens the sheet.
  var already = crew();
  if (already && already.token) enableCrewMode();
`;
}
...
function renderShotlist(shotlist, bundle, opts = {}) {
  ...
  const sceneMedia = bundle.sceneMedia || new Map();
  ...
  // Set design is the one field the crew can write from the link, so its row is
  // built differently from every other: when the shot list has a passcode the
  // row is rendered even when empty, hidden until someone unlocks, because a
  // scene with nothing dressed yet is exactly the one a set designer needs to
  // write on. Everything else on the page stays read-only here.
  const setRow = (label, value, target, id, max) => {
    if (!hasPasscode) return detail(label, value, 'set');
    return `<div class="row row-set${value ? '' : ' crew-only hidden'}" data-set-target="${esc(target)}" data-set-id="${Number(id)}" data-set-max="${Number(max)}">
              <div class="row-k">${icon('set')}${esc(label)}</div>
              <div class="row-v">
                <span class="set-text">${esc(value || '')}</span>
                <button type="button" class="set-edit crew-only hidden">${icon('set')}<span class="label">Edit</span></button>
              </div>
            </div>`;
  };

  // The scene's set design is repeated inside each of its shots as context. It
  // is not editable there, one field, one place to write it, but it is tagged
  // so a save on the scene updates every copy without a reload.
  const setEcho = (value, sceneId) => {
    if (!hasPasscode) return detail('Set design', value, 'set');
    return `<div class="row row-echo${value ? '' : ' hidden'}" data-set-echo="${Number(sceneId)}">
              <div class="row-k">${icon('set')}Set design</div>
              <div class="row-v"><span class="set-text">${esc(value || '')}</span></div>
            </div>`;
  };
    ...
    const blocks = timeline.items.map(item => {
      ...
      if (item.kind === 'move') {
        ...
        <section class="block move" data-start-min="${minuteOf(item.start_label)}" data-end-min="${minuteOf(item.end_label)}"
          ...
          <div class="block-parts">
            ...
            ${item.locked ? `<span class="part">${icon('locked')} Departs ${esc(item.locked_time)}</span>` : ''}
          </div>
      ...
      if (item.kind === 'break') {
        ...
        <section class="block meal" data-start-min="${minuteOf(item.start_label)}" data-end-min="${minuteOf(item.end_label)}"
          ...
          <div class="block-sub">${esc(fmtDuration(item.duration_minutes))}${item.location_name ? ` · at ${esc(item.location_name)}` : ''}</div>
          <div class="block-parts">
            ${item.fixed ? `<span class="part">${icon('locked')} Fixed time</span>` : ''}
            ${item.location_name ? `<span class="part">${icon('location')} ${esc(item.location_name)}</span>` : ''}
            ${item.placement_applies ? `<span class="part">${icon('move')} ${item.placement === 'before_move' ? 'Before the company move' : 'After the company move'}</span>` : ''}
          </div>
      ...
      // When this scene is on set, which is the only time the crew is being
      // asked to be anywhere. The light window is a constraint on planning it,
      // not a call time, read in the same row it was taken for one, so it is
      // now second and only shown when it actually constrains anything. A
      // scene that can be shot at any hour has no window worth printing.
      const onSetLabel = item.start_label && item.end_label
        ? `${item.start_label} to ${item.end_label}`
        : (item.start_label || '');
      const showWindow = !!item.light_window_label && scene.light_window !== 'any_time';
      ...
      // uncast part reads "The Child (casting 8 to 12)".
      const castLabel = c => {
        if (c.performer) return `${c.name} (${c.performer})`;
        const age = fmtAgeRange(c.age_min, c.age_max);
        return age ? `${c.name} (casting ${age})` : c.name;
      };
      // As above: what a plain reader sees, before the crew-editable set design
      // row is added on top of it.
      const sceneVisibleRows = scene.description || scene.set_design || cast.length;
      ...
      const shotCards = shots.map((shot, shotIdx) => {
        ...
        const photos = shotMedia.map(m => {
          ...
          const thumb = mediaSrc(m.thumb_filename || m.filename);
          if (!full) return '';
          // The grid shows the still thumb; the viewer opens the web copy, which
          // for an animated upload is an animated WebP that plays there.
          const animated = /\.webp$/i.test(m.filename || '');
          return `<figure><button type="button" data-full="${full}"><img src="${thumb}" alt="" loading="lazy"></button>` +
            `<figcaption>${m.kind === 'angle' ? 'Angle' : 'Reference'}${animated ? ' · GIF' : ''}</figcaption></figure>`;
        }).filter(Boolean).join('');
        ...
        // What a reader with only the link can see. The crew-editable set note
        // is rendered on top of this even when everything here is empty, so the
        // whole block is kept hidden until an unlock rather than drawing an
        // empty bordered box on every shot.
        const visibleRows = shot.description || characters || shot.costume || shot.props
          || shot.set_design || scene.set_design || lens || shot.camera_notes;
        const hasRows = visibleRows || hasPasscode;
        ...
        <article class="shot${isDone ? ' done' : ''}" data-shot="${Number(shot.id)}">
          ...
          ${hasRows ? `<div class="rows${visibleRows ? '' : ' crew-only hidden'}">
            ...
            ${setEcho(scene.set_design, item.scene_id)}
            ${setRow('Set note', shot.set_design, 'shot', shot.id, 2000)}
      ...
      <section class="scene" data-scene="${Number(item.scene_id)}"
        ...
        <div class="tags">
          ...
          ${onSetLabel
    ? `<span class="tag time">${icon('clock')} On set ${esc(onSetLabel)}</span>`
    : ''}
          ${showWindow
    ? `<span class="tag${item.light_window_hard ? ' hard' : ''}">${icon(winIcon)} Light window: ${esc(item.light_window_label)}${item.light_window_range ? ` ${esc(item.light_window_range)}` : ''}</span>`
    : ''}
        ...
        <div class="scene-body hidden" id="${bodyId}">
          ${sceneVisibleRows || hasPasscode ? `<div class="rows${sceneVisibleRows ? '' : ' crew-only hidden'}">
            ${detail('Scene', scene.description)}
            ${setRow('Set design', scene.set_design, 'scene', item.scene_id, 4000)}
            ${detail('Characters', cast.map(castLabel).join(', '), 'character')}
          </div>` : ''}
          ${scoutStrip(sceneMedia.get(item.scene_id))}
          ${wardrobeStrip(cast)}
          ${loc ? `
          <div class="loc">
            <div class="loc-text">
  ...
  <div class="sheet-bg hidden" id="sheet">
    <div class="sheet">
      ...
      <p>Enter the production passcode and your name to mark shots complete and write set design.</p>
      <form id="sheetForm">
        <div class="field">
  ...
  // Link preview. Every absolute URL is built from the origin the request came
  // in on, so a shot list opened on the public domain never advertises the
  // panel host in a preview card.
  const origin = opts.origin || '';
  const ogImage = origin && shotlist.slug
    ? `${origin}/shotlist/${encodeURIComponent(shotlist.slug)}/cover.png` : null;
  const ogUrl = origin && shotlist.slug
    ? `${origin}/shotlist/${encodeURIComponent(shotlist.slug)}` : null;
  // Shape only, a preview is shown in a chat list, so it says how big the job
  // is and never what is in it.
  const listTotals = bundle.totals || {};
  const ogDescription = [
    `${timelines.length} shoot day${timelines.length === 1 ? '' : 's'}`,
    `${listTotals.scene_count || 0} scene${listTotals.scene_count === 1 ? '' : 's'}`,
    `${listTotals.shot_count || 0} shot${listTotals.shot_count === 1 ? '' : 's'}`,
  ].join(' · ');
...
<!-- noindex still lets a link unfurler fetch the page and read the og: tags
     below, which is what makes a WhatsApp preview work without the shot list
     ever entering a search index. -->
<meta name="robots" content="noindex, nofollow">
<meta name="color-scheme" content="light dark">
<title>${esc(shotlist.title)}</title>
<meta property="og:title" content="${esc(shotlist.title)}">
