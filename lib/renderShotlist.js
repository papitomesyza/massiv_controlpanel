...
.scene{background:var(--panel);border:1px solid var(--line);border-radius:16px;margin-bottom:18px;overflow:hidden}
.scene-head{display:flex;gap:12px;padding:16px 14px 8px}
.scene-num{flex-shrink:0;min-width:48px;height:48px;border-radius:12px;background:var(--accent);color:var(--bg);
  display:flex;align-items:center;justify-content:center;font-size:19px;font-weight:800;font-variant-numeric:tabular-nums;padding:0 8px}
.scene-headline{flex:1;min-width:0}
.scene-label{font-size:11px;font-weight:800;letter-spacing:0.14em;text-transform:uppercase;color:var(--ink-3)}
.scene-title{font-size:24px;font-weight:800;letter-spacing:-0.02em;line-height:1.2;margin-top:1px;overflow-wrap:anywhere}
...
.scene-empty{padding:4px 14px 16px;font-size:15px;color:var(--ink-3)}

/* shot cards, nested inside their scene */
...
.num{flex-shrink:0;min-width:40px;height:40px;border-radius:10px;background:var(--chip);
  display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;font-variant-numeric:tabular-nums;padding:0 6px}
...
(function(){
  ...
  function refreshProgress(){
    ...
    if (nowText) {
      nowText.textContent = nextEl
        ? (nextEl.getAttribute('data-scene') + ' — ' + nextEl.getAttribute('data-title'))
        : 'All shots complete';
    }
...
// shotlist: row from shotlists
// rows: scheduled + decorated SCENE rows in the ORDER THE PANEL SELECTED
// shotsByScene: Map(scene id → its shots, in shot order)
// locationsById / mediaByShot: lookups for the location block and the photos
function renderShotlist(shotlist, rows, scenesById, shotsByScene, locationsById, mediaByShot, opts = {}) {
  const hasPasscode = !!shotlist.passcode_hash;

  const allShots = rows.flatMap(r => shotsByScene.get(r.scene_id) || []);
  const total = allShots.length;
  const doneCount = allShots.filter(s => s.status === 'completed').length;

  const detail = (label, value) => (value
    ? `<div class="row"><div class="row-k">${esc(label)}</div><div class="row-v">${esc(value)}</div></div>`
    : '');

  // One block per SCENE: where the crew is, when, and in what light — then the
  // coverage inside it.
  const cards = rows.map((row, sceneIdx) => {
    ...
    const sceneNumber = scene.scene_number || String(sceneIdx + 1);
    ...
    const spaceLabel = scene.space === 'interior' ? 'Interior' : 'Exterior';
    ...
    const shotCards = shots.map((shot, shotIdx) => {
      ...
      const number = shot.shot_number || String(shotIdx + 1);
      ...
      const mediaBlock = photos ? `
        <div class="media-wrap">
          <button type="button" class="media-toggle" data-target="media-${Number(shot.id)}" data-open-label="${esc(openLabel)}">${esc(openLabel)}</button>
      ...
      return `
      <article class="shot${isDone ? ' done' : ''}" data-shot="${Number(shot.id)}" data-title="${esc(title)}" data-scene="${esc(sceneTitle)}" data-time="${esc(row.start_label || '')}">
        <div class="shot-head">
          <div class="num">${esc(number)}</div>
          <div class="shot-headline">
            <div class="shot-title">${esc(title)}</div>
            ${shot.shot_type || shot.duration_minutes ? `<div class="shot-time">${shot.shot_type ? esc(shot.shot_type) : ''}${shot.duration_minutes ? `${shot.shot_type ? ' ' : ''}<span>· ${Number(shot.duration_minutes)} min</span>` : ''}</div>` : ''}
          </div>
        </div>
    ...
    return `
    <section class="scene" data-scene="${Number(row.scene_id)}">
      <div class="scene-head">
        <div class="scene-num">${esc(sceneNumber)}</div>
        <div class="scene-headline">
          <div class="scene-label">Scene</div>
          <h2 class="scene-title">${esc(sceneTitle)}</h2>
          <div class="scene-time">${esc(row.start_label || '')}${row.duration_minutes ? ` <span>· ${Number(row.duration_minutes)} min · ${shots.length} shot${shots.length === 1 ? '' : 's'}</span>` : ''}</div>
        </div>
      </div>
      <div class="tags">
      ...
      ${scene.description ? `<div class="rows">${detail('Scene', scene.description)}</div>` : ''}
      ${locBlock}
      ${shots.length
        ? `<div class="scene-shots">${shotCards}</div>`
        : '<p class="scene-empty">No shots in this scene yet.</p>'}
    </section>`;
  }).join('\n');
