...
// Allow is what lets a link unfurler fetch a page to build a preview card; the
// pages themselves carry noindex, so WhatsApp gets its thumbnail while a search
// engine still will not index them. Casting links go to named agencies rather
// than to the world, so those stay disallowed outright.
const ROBOTS_TXT = [
  'User-agent: *',
  'Allow: /p/',
  'Allow: /p-media/',
  'Allow: /shotlist/',
  'Allow: /shotlist-media/',
  'Disallow: /c/',
  'Disallow: /',
  '',
].join('\n');
...
// Public shot lists live at /shotlist/:slug with media at /shotlist-media, and
// are served on the public domain alongside pitches.
const SHOTLISTS_ON_PITCH_DOMAIN = true;
const SHOTLIST_SLUG_PATH = /^\/shotlist\/[^/]+\/?$/;
const SHOTLIST_MEDIA_PATH = /^\/shotlist-media\/[^/]+$/;
// The link preview cover, which unfurlers fetch as a plain image.
const SHOTLIST_COVER_PATH = /^\/shotlist\/[^/]+\/cover\.png$/;

function isShotlistPath(p) {
  return SHOTLIST_SLUG_PATH.test(p) || SHOTLIST_MEDIA_PATH.test(p) || SHOTLIST_COVER_PATH.test(p);
}

// Viewing a shot list needs only the link, but ticking a shot off needs the
// production passcode — and that exchange is a POST to the public API. These
// two endpoints are the ONLY thing under /api that exists on the public domain,
// and the only paths on that host where a non-GET method is allowed.
//
// Deliberately tight: the shot list public namespace, one slug segment, and
// then either the unlock action or a shot id followed by a completion action.
// Nothing else under /api/public matches — the expense endpoints in particular
// live in the same namespace and must stay unreachable here.
const CREW_API_PATH =
  /^\/api\/public\/shotlist\/[^/]+\/(unlock|shots\/[^/]+\/(complete|uncomplete))\/?$/;

function isCrewApiPath(p) {
  return CREW_API_PATH.test(p);
}
...
// than the panel's. It needs /shotlist-media too, since the cast photos and
// wardrobe stills live in the shot list media store — that store is already
// public on the panel domain, so this exposes nothing new.
const CASTING_SLUG_PATH = /^\/c\/[^/]+\/?$/;
const CASTING_COVER_PATH = /^\/c\/[^/]+\/cover\.png$/;

function isCastingPath(p) {
  return CASTING_SLUG_PATH.test(p) || CASTING_COVER_PATH.test(p) || SHOTLIST_MEDIA_PATH.test(p);
}
...
function pitchDomainGuard(req, res, next) {
  try {
    ...
    if (!isPitchHost(req)) {
      ...
      if (req.method === 'GET' || req.method === 'HEAD') {
        if (PITCH_SLUG_PATH.test(req.path) || PITCH_MEDIA_PATH.test(req.path)
          ...
          || (SHOTLISTS_ON_PITCH_DOMAIN && isShotlistPath(req.path))) {
          return res.redirect(301, `https://${PITCH_DOMAIN}${req.originalUrl}`);
        }
      }
      return next();
    ...
    // The one exception to "GET only": the crew endpoints, which are how a
    // shot list gets ticked off. POST only — a GET to these paths would fall
    // through to the SPA catch-all and hand back a panel asset, so it is not
    // allowed either, and every other method is refused outright.
    if (isCrewApiPath(req.path)) {
