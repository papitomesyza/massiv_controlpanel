      ...
      if (cast.length) {
        ...
        cast.filter(c => (c.wardrobe || []).length).forEach(c => {
          ...
          c.wardrobe.forEach(w => {
            ...
            const wp = safeMediaPath(w.thumb_filename || w.filename);
            if (wp) {
              try {
                doc.save();
                doc.rect(wx, y, W_W, W_H).clip();
                doc.image(wp, wx, y, { cover: [W_W, W_H], align: 'center', valign: 'center' });
                doc.restore();
              } catch (_) { doc.rect(wx, y, W_W, W_H).fill(HAIR); }
            } else {
              doc.rect(wx, y, W_W, W_H).fill('#FBFBFB');
            }
            if (w.label) {
