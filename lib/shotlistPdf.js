...
// ── Call sheet ────────────────────────────────────────────────────────────────
// Landscape A4, one row per shot, kept to as few pages as possible: rows grow
// only as tall as their tallest wrapped cell.

function callSheetPdf(res, { shotlist, rows, shotsById, locationsById, agency, orderLabel }) {
  ...
  const widths = {
    nr: 26, time: 46, space: 34, light: 104, location: 96,
    shot: 150, talent: 90, costume: 90, props: 142,
  };
  const order = ['nr', 'time', 'space', 'light', 'location', 'shot', 'talent', 'costume', 'props'];
  const labels = {
    nr: '#', time: 'TIME', space: 'I/E', light: 'LIGHT WINDOW', location: 'LOCATION',
    shot: 'SHOT', talent: 'TALENT', costume: 'COSTUME', props: 'PROPS',
  };
  const C = {};
  let cx = ML;
  ...
  rows.forEach((row, idx) => {
    ...
    const shotText = [shot.title || '', shot.shot_type || ''].filter(Boolean).join(' — ');
    const lightText = row.light_window_label
      ? `${row.light_window_label}${row.light_window_range ? `\n${row.light_window_range}` : ''}`
      : '';

    const values = {
      nr: shot.shot_number || String(idx + 1),
      time: row.start_label || '',
      space: shot.space === 'interior' ? 'INT' : 'EXT',
      light: lightText,
    ...
    order.forEach(k => {
      if (k === 'nr' || k === 'time') doc.font('Helvetica-Bold').fillColor(INK);
...
function photoBoardPdf(res, { shotlist, rows, shotsById, locationsById, mediaByShot, agency, orderLabel }) {
  const PDFDocument = require('pdfkit');
  const safeTitle = (shotlist.title || 'shotlist').replace(/[^a-z0-9]/gi, '-').slice(0, 40);
  ...
  rows.forEach((row, idx) => {
    const shot = shotsById.get(row.shot_id) || {};
    const loc = shot.location_id != null ? locationsById.get(shot.location_id) : null;
    const media = (mediaByShot.get(row.shot_id) || []).slice(0, 4);

    // Measure before drawing so a block is never split across pages.
    const detailLines = [
      ['Time', `${row.start_label || ''}${row.duration_minutes ? `  ·  ${row.duration_minutes} min` : ''}`],
      ['Light', row.light_window_label
        ? `${row.light_window_label}${row.light_window_range ? `  (${row.light_window_range})` : ''}` : ''],
      ['Space', shot.space === 'interior' ? 'Interior' : 'Exterior'],
      ['Type', shot.shot_type || ''],
    ...
    doc.fontSize(11).font('Helvetica-Bold').fillColor(INK)
      .text(`${shot.shot_number || idx + 1}.  ${shot.title || 'Untitled shot'}`, TEXT_X, ty, { width: TEXT_W });
    ty = doc.y + 4;
