  ...
  bundle.timelines.forEach((timeline, dayIndex) => {
    ...
    timeline.items.forEach(item => {
      ...
      const values = {
        ...
        // A scene that can be shot at any hour has no window worth printing;
        // "00:00 to 23:59" under a LIGHT WINDOW heading only invites a misread.
        light: item.light_window_label && scene.light_window !== 'any_time'
...
function photoBoardPdf(res, { shotlist, bundle, agency, orderLabel }) {
  ...
  bundle.timelines.forEach((timeline, dayIndex) => {
    ...
    timeline.items.forEach(item => {
      ...
      const bits = [
        item.start_label
          ? `On set ${item.start_label}${item.end_label ? ` to ${item.end_label}` : ''}` : '',
        scene.space === 'interior' ? 'Interior' : 'Exterior',
        // Named as light so it is never read as a call time, and dropped
        // entirely when the scene can be shot at any hour.
        item.light_window_label && scene.light_window !== 'any_time'
          ? `Light window: ${item.light_window_label}${item.light_window_range ? ` ${item.light_window_range}` : ''}` : '',
      ...
      // What the recce found here. It belongs to the scene, above its shots,
      // because it describes the place rather than any one framing.
      const scoutShots = (bundle.sceneMedia && bundle.sceneMedia.get(item.scene_id)) || [];
      if (scoutShots.length) {
        if (needsPage(90)) { doc.addPage(); newPageSetup(); y = MT; }
        y += 4;
        doc.fontSize(7).font('Helvetica-Bold').fillColor(MUTED)
          .text('SCOUT PHOTOS', ML, y, { lineBreak: false });
        y += 10;
        const S_W = 96;
        const S_H = 68;
        let sx = ML;
        scoutShots.forEach(m => {
          if (sx + S_W > PW - MR) { sx = ML; y += S_H + 16; }
          drawClippedImage(doc, pdfImagePath(m), sx, y, S_W, S_H, HAIR);
          if (m.label) {
            doc.fontSize(6).font('Helvetica').fillColor(MUTED)
              .text(fitText(doc, m.label, S_W), sx, y + S_H + 2, { width: S_W, lineBreak: false });
          }
          sx += S_W + 8;
        });
        y += S_H + 16;
      ...
      shots.forEach((shot, idx) => {
        ...
        media.forEach(m => {
          ...
          doc.fontSize(6.5).font('Helvetica-Bold').fillColor(MUTED)
            .text(m.kind === 'angle' ? 'ANGLE' : 'REFERENCE', ML, iy + IMG_H + 2, { width: IMG_W, lineBreak: false });
          iy += IMG_H + 12;
        });
        if (media.length === 0) {
