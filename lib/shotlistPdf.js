...
const fs = require('fs');
const { fmtDuration, fmtClip } = require('./shotlistSchedule');
...
function characterNames(shot, charactersByShot) {
  return (charactersByShot.get(shot.id) || [])
    .map(c => (c.performer ? `${c.name} (${c.performer})` : c.name))
    .join(', ');
}
...
function drawDayBand(doc, { day, totals }, ML, MR, PW, y, index) {
  ...
  cells.forEach(([label, short, value], i) => {
    ...
    doc.fontSize(6.5).font('Helvetica-Bold').fillColor(MUTED)
      .text(doc.widthOfString(label) <= usable ? label : short, x, y + 6, { width: usable, lineBreak: false });
    doc.fontSize(9).font('Helvetica-Bold').fillColor(INK)
      .text(value, x, y + 17, { width: usable, lineBreak: false });
  });
  return y + H + 8;
}
    ...
    timeline.items.forEach(item => {
      ...
      if (item.kind === 'move' || item.kind === 'break') {
        ...
        const text = item.kind === 'move'
          ...
          : `${String(item.break_kind || 'break').toUpperCase()}   ${item.label}   ${item.duration_minutes} min${item.fixed ? '   [FIXED]' : ''}`;
    ...
    timeline.items.forEach(item => {
      if (item.kind === 'move' || item.kind === 'break') {
        ...
        const text = item.kind === 'move'
          ...
          : `${item.start_label}   ${item.label.toUpperCase()}   ${item.duration_minutes} min${item.fixed ? '   [FIXED]' : ''}`;
      ...
      if (cast.length) {
        ...
        cast.forEach(c => {
          ...
          const p = safeMediaPath(c.photo_thumb_filename || c.photo_filename);
          if (p) {
            try {
              doc.save();
              doc.rect(cx2, y, CH_W, CH_H).clip();
              doc.image(p, cx2, y, { cover: [CH_W, CH_H], align: 'center', valign: 'center' });
              doc.restore();
            } catch (_) { doc.rect(cx2, y, CH_W, CH_H).fill(HAIR); }
          } else {
            doc.rect(cx2, y, CH_W, CH_H).fill('#FBFBFB');
          }
          doc.fontSize(7).font('Helvetica-Bold').fillColor(INK)
            .text(c.name, cx2, y + CH_H + 2, { width: CH_W, lineBreak: false });
          if (c.performer) {
            doc.fontSize(6.5).font('Helvetica').fillColor(MUTED)
              .text(c.performer, cx2, y + CH_H + 11, { width: CH_W, lineBreak: false });
          }
          cx2 += CH_W + 8;
        });
        y += CH_H + 24;
        ...
        media.forEach(m => {
          ...
          if (p) {
            try {
              ...
              doc.restore();
            } catch (_) { doc.rect(ML, iy, IMG_W, IMG_H).fill(HAIR); }
          } else {
