...
// A shot list linked to a project follows the project's shoot date. Called
// inside the transaction that changes the project date, from every path that
// changes it. Only a single day list moves, and it moves as a unit: when its
// day still reads the old project date or has none, the day takes the new
// date, and so does the list's own shoot_date when it too reads the old date
// or none. A day set to another date by hand keeps it, and a list with more
// than one day is never moved. Returns the ids of the lists that changed.
function followProjectShootDate(projectId, oldDate, newDate) {
  const from = oldDate ? String(oldDate).slice(0, 10) : null;
  const to = newDate ? String(newDate).slice(0, 10) : null;
  if (!to || from === to) return [];
  const follows = d => !d || String(d).slice(0, 10) === from;
  const moved = [];
  const lists = db.prepare('SELECT id, shoot_date FROM shotlists WHERE project_id = ?').all(projectId);
  lists.forEach(list => {
    const days = db.prepare('SELECT id, shoot_date FROM shotlist_days WHERE shotlist_id = ?').all(list.id);
    if (days.length > 1) return;
    const lead = days.length === 1 ? days[0].shoot_date : list.shoot_date;
    if (!follows(lead)) return;
    if (days.length === 1) {
      db.prepare('UPDATE shotlist_days SET shoot_date = ? WHERE id = ?').run(to, days[0].id);
    }
    if (follows(list.shoot_date)) {
      db.prepare('UPDATE shotlists SET shoot_date = ? WHERE id = ?').run(to, list.id);
    }
    touch(list.id);
    moved.push(list.id);
  });
