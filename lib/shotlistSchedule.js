...
function moveParts(shotlist, toScene) {
  ...
  return {
    wrap: Number.isFinite(Number(wrapOverride)) && wrapOverride !== null ? Number(wrapOverride) : listWrap,
    setup: Number.isFinite(Number(setupOverride)) && setupOverride !== null ? Number(setupOverride) : listSetup,
    overridden: (wrapOverride !== null && wrapOverride !== undefined)
      || (setupOverride !== null && setupOverride !== undefined),
  };
}
...
function buildDayTimeline(shotlist, day, scenes, shotsByScene, breaks, locations, leg, opts = {}) {
  ...
  // running order. sort_order n means "before the nth scene".
  ...
  function placeBreaks(index) {
    (breaksBefore.get(index) || []).forEach(b => {
  ...
  scenes.forEach((scene, index) => {
    ...
    if (currentLoc != null && scene.location_id != null && scene.location_id !== currentLoc) {
      ...
      const total = parts.wrap + travel.minutes + parts.setup;
    ...
    let start = cursor + (move ? move.duration_minutes * MIN : 0);
    ...
    if (lockMs != null) {
      ...
      if (move && start > lockMs) {
        warnings.push({
          type: 'move_impossible',
          day_id: day ? day.id : null,
          scene_id: scene.id,
          message: `${sceneName(scene, index)} is locked to ${fmtTime(lockMs)} but the company move from ${move.from_name || 'the previous location'} needs ${move.duration_minutes} min and cannot arrive before ${fmtTime(start)}.`,
        });
      }
      start = lockMs;
    ...
    if (move) {
      move.end_ms = start;
      move.start_ms = start - move.duration_minutes * MIN;
