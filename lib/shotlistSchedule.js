  ...
  scenes.forEach((scene, index) => {
      ...
      beforeMove.forEach(b => {
        ...
        if (fixed != null && fixed > wrapBeganAt) {
          warnings.push({
            ...
            day_id: day ? day.id : null,
            break_id: b.id,
            message: `${b.label || defaultBreakLabel(b.kind)} is set before the move at ${fmtTime(fixed)}, but the crew wraps ${move.from_name || 'the previous location'} at ${fmtTime(wrapBeganAt)}, so it would be eaten after the wrap has already begun.`,
          });
        }
      });
    }
    ...
    if (win && win.key !== 'any_time' && (start < win.from || end > win.to)) {
      warnings.push({
        ...
        scene_id: scene.id,
        message: lockMs != null
          ? `${sceneName(scene, index)} is locked to ${fmtTime(start)}, outside its ${windowLabel(win.key)} window (${win.range_label}), so the light will not match.`
          : `${sceneName(scene, index)} is scheduled ${fmtTime(start)} to ${fmtTime(end)} but its ${windowLabel(win.key)} window is ${win.range_label}.`,
      });
    }
...
// "30 to 40", "40+", "under 12", "35", the age a role is cast for, written the
