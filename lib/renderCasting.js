...
// for, and the schedule the part is booked on. Wardrobe is deliberately absent:
// what somebody wears is the production's business, not the agency's, and it
// lives on the crew page and the photo board instead.
//
...
// shotCounts maps character id → how many shots
...
function renderCasting(shotlist, characters, opts = {}) {
  ...
  const card = c => {
    ...
    // Only the casting photo leads the card. The lead image reads as the person
    // being put forward for the part, so nothing else may stand in for it —
    // no photo says no photo.
  ...
  // Link preview. Absolute URLs come from the origin the request arrived on, so
  // a casting opened on the public domain never advertises the panel host.
  const origin = opts.origin || '';
  const ogImage = origin && shotlist.casting_slug
    ? `${origin}/c/${encodeURIComponent(shotlist.casting_slug)}/cover.png` : null;
  const ogUrl = origin && shotlist.casting_slug
    ? `${origin}/c/${encodeURIComponent(shotlist.casting_slug)}` : null;
  // Shape only — the preview is seen in a chat list, so it says how big the
  // casting is and nothing about who is in it.
  const ogDescription = [
    `${total} part${total === 1 ? '' : 's'}`,
    principals.length ? `${principals.length} principal${principals.length === 1 ? '' : 's'}` : '',
...
<!-- noindex still lets a link unfurler fetch the page and read the og: tags
     below, which is what makes a WhatsApp preview work without the casting
     ever entering a search index. -->
<meta name="robots" content="noindex, nofollow">
<title>${esc(shotlist.title || 'Casting')}${agencyName ? ` · ${esc(agencyName)}` : ''}</title>
<meta property="og:title" content="${esc(shotlist.title || 'Casting')}">
