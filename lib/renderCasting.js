...
function renderCasting(shotlist, characters, opts = {}) {
  ...
  const card = c => {
    ...
    // A grid of empty rectangles is no use to an agency: where there is no
    // casting photo, the first wardrobe still leads the card instead.
    const lead = c.photo_filename
      ? { full: c.photo_filename, thumb: c.photo_thumb_filename || c.photo_filename }
      : ((c.wardrobe || [])[0]
        ? { full: c.wardrobe[0].filename, thumb: c.wardrobe[0].thumb_filename || c.wardrobe[0].filename }
        : null);
    const photo = lead ? mediaSrc(lead.full) : null;
    const photoThumb = lead ? mediaSrc(lead.thumb) : null;
  ...
  const withPhotos = characters.filter(c => c.photo_filename || (c.wardrobe || []).length).length;
...
<header class="head">
  <div class="wrap">
    ...
    <div class="meta">
      ...
      ${withPhotos ? `<span class="chip">${withPhotos} with reference</span>` : ''}
    </div>
