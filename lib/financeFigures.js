...
function owedSummary(f = {}, today = pristinaToday()) {
  const rows = owedByProject(f, today);
...
function crewAssignmentFigures(f = {}, opts = {}) {
  ...
  `).all(...params).map(r => ({
    ...r,
    agreed: round2(r.agreed),
    paid: round2(r.paid),
    remaining: round2(r.remaining),
    total_cost: round2(r.agreed),
  }));
}
...
function crewOwedSummary(f = {}, opts = {}) {
  ...
  const t = rows.reduce((a, r) => {
    a.agreed += r.agreed; a.paid += r.paid; a.remaining += r.remaining;
    return a;
  }, { agreed: 0, paid: 0, remaining: 0 });
  return { agreed: round2(t.agreed), paid: round2(t.paid), remaining: round2(t.remaining), rows };
}
